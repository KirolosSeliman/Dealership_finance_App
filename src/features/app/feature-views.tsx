...
export function ChartPanel({ title, data, type, summary }: { title: string; data: { label: string; value: number }[]; type: "area" | "bar"; summary: string }) {
...
export function deepCaptureStatusLabel(status?: string) {
...
export function DealRadarTable({ t, items, canManage, onView, onRemove, onConvert }: { t: ReturnType<typeof getDictionary>; items: Array<Record<string, unknown>>; canManage: boolean; onView: (item: Record<string, unknown>) => void; onRemove: (item: Record<string, unknown>) => void; onConvert: (item: Record<string, unknown>) => void }) {
  return (
    <div className="overflow-x-auto">
      <table className="data-table">
        <thead>
...
export function DealRadarCard({ t, item, canManage, onView, onRemove, onConvert }: { t: ReturnType<typeof getDictionary>; item: Record<string, unknown>; canManage: boolean; onView: () => void; onRemove: () => void; onConvert: () => void }) {
  return (
...
export function DealRadarAnalysis({ t, item }: { t: ReturnType<typeof getDictionary>; item: Record<string, unknown> }) {
...
export function InfoGrid({ rows }: { rows: Array<[React.ReactNode, React.ReactNode]> }) {
...
export function Ledger({ rows, emptyTitle = "No records yet", emptyCopy = "Records will appear here once activity is created." }: { rows: React.ReactNode[][]; emptyTitle?: string; emptyCopy?: string }) {
...
export function tabLabel(t: ReturnType<typeof getDictionary>, key: string) {
...
export function isInDateRange(date: string, range: { start: string; end: string }) {
  if (range.start && date < range.start) return false;
  if (range.end && date > range.end) return false;
  return true;
}

export function buildDailySeries(rows: Array<{ date: string; value: number }>) {
...
export function buildBalanceSeries<T extends CashTransaction>(
  transactions: T[],
...
export function buildMonthlyBuySellSeries(scoped: AppData, range: { start: string; end: string }) {
...
export function dealRadarLabel(item: Record<string, unknown>) {
  return [item.year, item.make, item.model, item.trim].filter(Boolean).join(" ") || String(item.title ?? "Unknown listing");
}

export function normalizedExpenseAmount(expense: VehicleExpense) {
...
export function sumSeries(series: Array<{ value: number }>) {
  return roundDisplayNumber(series.reduce((sum, item) => sum + item.value, 0));
}

export function lastSeriesValue(series: Array<{ value: number }>) {
...
export function roundDisplayNumber(value: number) {
