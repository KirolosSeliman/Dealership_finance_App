...
import type { VehiclePrefill } from "@/features/app/feature-views";
...
export function DealerFlowApp() {
  ...
  async function voidExpense(expenseId: string, reason: string) {
    if (!supabase || !selectedVehicle) return;
    const vehicleSnapshot = selectedVehicle;
    setLoading(true);
    setErrorMessage("");
    try {
      await serverMutation("voidExpense", newMutationForm({ organizationId: vehicleSnapshot.organizationId, vehicleId: vehicleSnapshot.id, expenseId, reason }));
      await refreshData(vehicleSnapshot.organizationId);
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : "Could not void expense.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function archiveVehicle(reason: string) {
    if (!supabase || !selectedVehicle) return;
    const vehicleSnapshot = selectedVehicle;
    setLoading(true);
    setErrorMessage("");
    ...
    try {
      ...
      setStatusMessage(result.warning || "Vehicle archived. Financial and tax history was preserved.");
      await refreshData(vehicleSnapshot.organizationId);
    } catch (error) {
